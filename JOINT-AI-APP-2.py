...
if st.session_state['model_tq']:
    ...
    with tab2:
        ...
        with sim_l:
            ...
            # [안전장치] 슬라이더 및 인풋 그리기 전에 값 유효 범위 강제 제어 (Clamping)
            st.session_state['sim_cd'] = max(0.0, min(float(st.session_state['sim_cd']), 15.0))
            st.session_state['sim_sc'] = max(0.0, min(float(st.session_state['sim_sc']), 10.0))

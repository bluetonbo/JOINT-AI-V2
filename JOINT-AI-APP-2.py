...
if 'model_tq' not in st.session_state:
    st.session_state.update({
        ...
        'data_bounds': {  
            'Caulking_Distance': (4.0, 7.0), 'Stud_Center': (1.5, 3.5),
            'Aging_Status': (0, 1), 'S_L': (0.0, 10.0), 'T_R': (0.0, 10.0)
        },
...
if st.session_state['model_tq']:
    ...
    with tab1:
        # 기존 로직 유지 + S_L, T_R 범위 반영
        if st.button("RUN INVERSE INFERENCE SEARCH"):
            res = minimize(lambda x: 0, [5, 2, 0, 5, 5], method='SLSQP') # 로직 유지
